...
import { FaTimes, FaRocket, FaLock, FaGlobe, FaShieldAlt, FaEdit, FaReply, FaUserFriends, FaCoins, FaCamera, FaUpload, FaClock, FaUsers, FaCrown, FaGem, FaHeart, FaMapMarkerAlt, FaPollH, FaMicrophone, FaVideo, FaComment, FaRecordVinyl, FaDesktop, FaBroadcastTower, FaSmile, FaShare, FaTrash } from 'react-icons/fa';
import { useGroupStore, useGroupActions, GROUP_TYPES, PRIVACY_TYPES } from '../../../lib/groupStore';
import { useApp } from '../../Context/AppContext';

const InstantRoomCreator = ({ onClose, onCreated, isCompact = false }) => {
  const { theme } = useApp();
  ...
  const getPrivacyIcon = (privacy) => {
    ...
    switch (privacy) {
      ...
      case PRIVACY_TYPES.SECRET: return <FaShieldAlt className={`${iconClass} text-purple-500`} />;
      ...
      <div className={`h-full flex flex-col ${theme.bgColor} ${theme.textColor}`}>
        ...
        <div className={`flex items-center justify-between p-4 border-b ${theme.borderColor} bg-gradient-to-r ${theme.headerBg}`}>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold">Instant-Room</h3>
              <p className={`text-xs ${theme.secondaryText}`}>Étape {step} sur 4</p>
            </div>
          </div>
        ...
        <div className={`h-1 bg-gray-200 ${theme.borderColor}`}>
          <motion.div
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <AnimatePresence mode="wait">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full p-2 text-sm rounded-lg border ${theme.borderColor} ${theme.inputBg} focus:outline-none focus:ring-1 ${theme.focusRing}`}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className={`w-full p-2 text-sm rounded-lg border ${theme.borderColor} ${theme.inputBg} resize-none focus:outline-none focus:ring-1 ${theme.focusRing}`}
                      />
                    </div>
                  </div>
                </div>
            ...
            {step === 2 && (
              ...
              >
                <div>
                  <h4 className="text-sm font-semibold mb-3">Confidentialité</h4>
                      ...
                      <button
                        ...
                        className={`w-full p-2 rounded-lg border-2 transition-all text-left ${
                          roomData.privacy === option.value
                            ? `border-orange-500 bg-gradient-to-r ${option.color} text-white`
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3">
                    <div>
                      <label className="block text-xs font-medium mb-1">Max membres</label>
                      <input
                        type="number"
                        min="2"
                        max="1000"
            ...
            {step === 4 && (
              ...
              >
                <div>
                  <h4 className="text-sm font-semibold mb-3">Monétisation</h4>
                  ...
                  <label className="flex items-center gap-2 p-2 rounded-lg cursor-pointer hover:bg-gray-50 text-xs">
                    ...
                    />
                    <span>Activer la monétisation</span>
                  </label>

                  {roomData.monetization.enabled && (
                    <div className="space-y-2 p-2 rounded-lg bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200">
                      <div>
                        <label className="block text-xs font-medium mb-1">Type</label>
                        <select
                          value={roomData.monetization.type}
                          onChange={(e) => setRoomData(prev => ({ 
                            ...prev, 
                            monetization: { ...prev.monetization, type: e.target.value }
                          }))}
                          className={`w-full p-2 text-xs rounded-lg border ${theme.borderColor} ${theme.inputBg} focus:outline-none focus:ring-1 ${theme.focusRing}`}
                        >
                      ...
                      <div>
                        <label className="block text-xs font-medium mb-1">Prix (Elite-Coins)</label>
                        <input
                          type="number"
                ...
                <div>
                  ...
                  <div className={`p-3 rounded-lg border ${theme.borderColor} ${theme.itemHover} text-xs`}>
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-gray-600">{roomData.description}</div>
                        <div className="flex items-center gap-2 mt-1 text-gray-500">
                          ...
                          {roomData.monetization.enabled && (
                            <>
                              <span>•</span>
                              <span className="text-yellow-600">{roomData.monetization.price} Elite-Coins</span>
                            </>
                          )}
                        </div>
                      </div>
        ...
        <div className={`p-3 border-t ${theme.borderColor} bg-gradient-to-r ${theme.headerBg}`}>
          <div className="flex items-center gap-2">
            ...
            <motion.button
              ...
              disabled={!canProceed()}
              className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ml-auto ${
                canProceed()
            ...
            >
              {step === 4 ? (
                <>
                  <FaRocket className="w-3 h-3 inline mr-1" />
                  Créer
                </>
              ) : (
                'Suivant'
